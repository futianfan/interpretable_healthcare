...
python ./src/neural_net.py results/rule_data_list ./data/train_lstm_output.npy ./data/snow.Y  ./data/test_lstm_output.npy ./data/test_snow.Y  
...
class RLP(torch.nn.Module):
    ...
    def forward(self, X_batch):   
        ...
        for j in range(PROTOTYPE_NUM):
        	pp_out = p_out[:,j].contiguous()
        	pp_out.view(1,-1)
        	for k in range(HIDDEN_SIZE):
        		##print(str(float(p_out[k,j]))[:7], end = ' ')
        		pass        
        	##print()
        #### print prototype
        ...
        for j in range(batch_size):
        	pp_out = X_out4[j,:]
        	for k in range(PROTOTYPE_NUM):
        		#print(str(float(X_out4[j,k].data))[:6], end = ' ')
        		pass
        	#print()
        #print(X_out4)
        ###### prototype
