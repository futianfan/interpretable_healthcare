'''
python ./src/neural_net-v20.py data/training_model_by_word2vec_1.vector data/tmp3 ./data/snow.Y ./data/test_data_1_3.txt ./data/test_snow.Y ./data/key_factor
'''
...
lamb = 0.1
################################################################################################################
f_key_factor = open(sys.argv[6], 'r')
...
def calculate_statistics(data_item, key_factor_indx):
    ...
    ff = lambda x: 1 if x in data_item else 0
    for j in range(ll):  stat[j] += ff(j)
    ret = np.array([i * 1.0 for i in stat])
    ##print(ret)
...
class RLP(torch.nn.Module):
    def __init__(self, INPUT_SIZE, HIDDEN_SIZE, NUM_LAYER, OUT_SIZE, interpret_OUT_SIZE,  BATCH_FIRST = True):
        ...
        ## interpretable
        self.interpret_network = nn.Linear(HIDDEN_SIZE, interpret_OUT_SIZE)
    ...
    def forward(self, X_batch, X_len):
        ...
        ### full connected
        X_out_classify = F.softmax(self.out3(X_out2))
        X_out_interpret = F.sigmoid(self.interpret_network(X_out2))
        return X_out_classify, X_out_interpret
...
for epoch in range(EPOCH):
    ...
    for i in range(iter_in_epoch):
        ...
        loss_interpret = 0 
        target_value = np.zeros((endn - stt, interpret_OUT_SIZE), dtype = float)
        for j in range(endn - stt):
            target_value[j,:] = calculate_statistics(data_dict[stt + j], key_indx)
            ##print(target_value[j,:])
        target_value = Variable(torch.from_numpy(target_value).float())
        ### interpretable network

        output1, output2 = nnet(batch_x, batch_len)
        loss = loss_crossentropy(output1, batch_label)  + lamb * loss_mse(output2, target_value)
        opt_.zero_grad()
